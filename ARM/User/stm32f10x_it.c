...
extern unsigned char RxBuffer_index;
extern unsigned char RxBuffer_index_next;
/*******************************************************************************
...
void USART1_IRQHandler(void)
{
  if(USART_GetITStatus(USART1, USART_IT_RXNE) != RESET)
  {
    /* Read one byte from the receive data register */
        ...
        if(RxBuffer_index == 7) RxBuffer_index_next = 0;
        else RxBuffer_index_next = RxBuffer_index + 1;
	if(RxBuffer[RxBuffer_index] == 0xff && RxBuffer[RxBuffer_index_next] == 0xfe)
	{
          cmd_flag = 0x1;
          USART_ITConfig(USART1, USART_IT_RXNE, DISABLE);
	}
        RxBuffer_index = RxBuffer_index_next;
